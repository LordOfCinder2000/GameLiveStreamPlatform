...
export interface OidcStoreSettings {
	...
	defaultSigninPasswordOptions?: OidcSigninPasswordOptions;
...
export interface OidcStoreActions {
	...
	oidcSignInSilentCallback: (payload?: object) => Promise<void> | undefined;
	authenticateOidcPassword: (
		payload?: SigninPasswordArgs
...
export abstract class OidcStore {
	...
	protected _getters: OidcStoreGetters = {
		...
		oidcScopes: (state: OidcStoreState) => {
			if (state.user && state.user.scope)
				return state.user.scope.split(",");
			return [];
		},
