...
export const OidcHelpers = {
	GetOidcCallbackPath: (callbackUri: string, routeBase = "/") => {
		if (callbackUri) {
			...
			return hostAndPath
				.substring(
					hostAndPath.indexOf(routeBase) + routeBase.length - 1
				)
				.replace(/\/$/, "");
		}
		return null;
	...
	CreateUserManager: (oidcSettings: StoreOidcClientSettings) => {
		...
		const oidcSettingsChange = Object.assign({}, oidcSettings);
		oidcSettingsChange.automaticSilentRenew = false;
		return new UserManager(oidcSettingsChange);
	},
	...
	AddUserManagerEventListener: (
		...
		if (
			typeof oidcUserManager.events[
				addFnName as TUserManagerEventsMethod
			] === "function" &&
	...
	RemoveUserManagerEventListener: (
		...
		const removeFnName =
			"remove" + OidcUtils.FirstLetterUppercase(eventName);
		if (
			typeof oidcUserManager.events[
				removeFnName as TUserManagerEventsMethod
			] === "function" &&
			typeof eventListener === "function"
		) {
	...
	processSignInCallback: (oidcSettings: OidcClientSettings) => {
		return new Promise((resolve, reject) => {
			...
			oidcUserManager
				...
				.then(() => {
					resolve(
						sessionStorage.getItem("store_oidc_active_route") || "/"
					);
				})
				.catch((err: Error) => {
					reject(err);
