<template>
	<div
		class="carousel-3d-container fit"
	...
	>
		...
		></Controls3dCarousel>
		<q-resize-observer debounce="300" @resize="onResizeCarousel" />
	</div>
</template>
...
export default {
	...
	computed: {
		...
		slideWidth() {
			...
			return vw < sw && process.env.CLIENT ? vw : sw;
		},
		slideHeight() {
	...
	methods: {
		onResizeCarousel(size) {
			this.setCarouseSpace();
		},
		setCarouseSpace() {
			let controlCarouselW = this.$el.children[1].children[0].offsetWidth;
			let carouselSpace = parseInt(
				(this.$el.offsetWidth - this.width - controlCarouselW) /
					2 /
					1.5,
				10
			);
			this.spaceDynamic = carouselSpace;
		},
		/**
		 * Go to next slide
		 */
		...
		getSlideCount() {
			...
			var a = this.$slots.default()[0].tag;
			if (this.$slots.default()) {
				children = this.$slots
					.default()
					.filter(
						(children) =>
							children.type.toString() !==
							Symbol("Comment").toString()
					);
				let childrenVFor = children.filter(
					(i) => i.type.toString() === Symbol("Fragment").toString()
				)[0].children.length;
				if (childrenVFor > 0)
					children = children.length + childrenVFor - 1;
			}
